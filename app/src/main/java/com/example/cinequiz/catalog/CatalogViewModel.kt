...
import com.example.cinequiz.model.discoverSeries.ListSeriesGenre
...
import com.example.cinequiz.model.popularSeriesModel.PopularSeriesList
...
class CatalogViewModel : ViewModel() {
    ...
    val seriesLiveData: MutableLiveData<List<PopularSeriesList>> by lazy { MutableLiveData<List<PopularSeriesList>>() }
    val seriesCarouselLiveData: MutableLiveData<List<PopularSeriesList>> by lazy { MutableLiveData<List<PopularSeriesList>>() }

    val seriesGenreLiveData: MutableLiveData<List<ListSeriesGenre>> by lazy { MutableLiveData<List<ListSeriesGenre>>() }
    val seriesCarouselGenreLiveData: MutableLiveData<List<ListSeriesGenre>> by lazy { MutableLiveData<List<ListSeriesGenre>>() }
    ...
    var seriesGenreNextPage = 0
    var seriesNextPage = 0
...
//    }
    ...
    fun seriesFirstRequisition() = viewModelScope.launch {
        try {
            ...
            repository.getPopularSeries().let { series->
                ...
                seriesLiveData.postValue(separation[1])
                seriesCarouselLiveData.postValue(separation[0])
    ...
    private fun seriesNextPage(series: PopularSeries) {
        seriesNextPage = series.page?.plus(1) ?: 1
    }

    fun seriesAnotherPage() = viewModelScope.launch {
        try {
            pagingLoading.postValue(true)
            repository.getPopularSeries(page = seriesNextPage).let {
                seriesNextPage(it)
                seriesLiveData.postValue(it.listMovies)
            }
    ...
    fun seriesGenreFirstRequisition() = viewModelScope.launch {
        try {
            pagingLoading.postValue(true)
            repository.getDiscoverSeries(genre = genre).let { series ->
                seriesGenreNextPage(series)

                val separation = separation(series.listSeriesGenres)

                seriesGenreLiveData.postValue(separation[1])
                seriesCarouselGenreLiveData.postValue(separation[0])
    ...
    private fun seriesGenreNextPage(series: SeriesByGenre) {
        seriesGenreNextPage = series.page?.plus(1) ?: 1
    }

    fun seriesGenreAnotherPage() = viewModelScope.launch {
        try {
            pagingLoading.postValue(true)
            repository.getDiscoverSeries(page = seriesGenreNextPage, genre = genre).let {
                seriesGenreNextPage(it)
                seriesGenreLiveData.postValue(it.listSeriesGenres)
            }
